...
public class ProfissionalDAO {
    ...
    public boolean gravar(Profissional profissional, Conexao conexao) {
        String sql = """
            INSERT INTO profissional(profissional_rn, profissional_tipo, profissional_dataadmissao, profissional_pessoa_cpf, profissional_graduacao_id, profissional_usuario, profissional_senha)
            VALUES (#1, #2, '#3', '#4', #5, '#6', '#7')
        """;
        sql = sql.replace("#1", String.valueOf(profissional.getProfissional_rn()));
    ...
    public boolean alterar(Profissional profissional, Conexao conexao) {
        ...
        sql = sql.replace("#7", String.valueOf(profissional.getProfissional_rn()));
    ...
    public List<Profissional> get(Conexao conexao, List<Map<String, Object>> pessoas, List<Map<String, Object>> graduacoes) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                profissional.setProfissional_rn(rs.getInt("profissional_rn"));
    ...
    public Profissional getProfissional(Conexao conexao, Profissional profissional,Map<String, Object> pessoa,Map<String, Object> graduacao) {
        String sql = "SELECT * FROM profissional " +
                ...
                "JOIN endereco ON pessoa_enderecoid = endereco_id " +
                "JOIN graduacao ON profissional_graduacao_id = graduacao_id "+
                "WHERE profissional_rn=#1 ORDER BY pessoa_nome;";
        sql=sql.replace("#1",""+profissional.getProfissional_rn());
